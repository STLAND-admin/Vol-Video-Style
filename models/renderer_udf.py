...
class DeformUDFRenderer:
    ...
    def render_core(self,
        ...
        with torch.enable_grad():
            pts_canonical = pts_canonical.requires_grad_(True)
            out = sdf_network(pts_canonical, ambient_coord, dirs_o, appearance_code, None)
            rgb = out['rgb'].reshape(batch_size, n_samples, 3)
            sdf = out['alpha'].reshape(batch_size, n_samples, 1)
            gradients_o = autograd.grad(
                sdf,
                pts_canonical,
                torch.ones_like(sdf, device=pts_canonical.device),
                create_graph=True,
                retain_graph=True,
                only_inputs=True)[0]
        ...
        beta_inv = 1 / beta
        density = beta_inv * (0.5 + 0.5 * sdf.sign() * torch.expm1(-sdf.abs() / beta))

        # LOG SPACE
        free_energy = dists * density.squeeze()
