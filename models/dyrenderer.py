...
class DeformNeusRenderer_dycheck:
    def __init__(self,
                 report_freq,
                 deform_network,
                 ambient_network,
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        ...
        # inside_sphere = (radius[:, :-1] < 100.0) | (radius[:, 1:] < 100.0)
    ...
    def cat_z_vals(self, time_id, deform_code, rays_o, rays_d, z_vals, new_z_vals, sdf, last=False,
                alpha_ratio=0.0):
        batch_size, n_samples = z_vals.shape
        ...
        if not last:
            ...
            pts = pts.reshape(-1, 3)
            # Deform
            deform_code_embed = deform_code(time_id.repeat(1,new_z_vals.shape[1]))

            deform_code_embed = deform_code_embed.reshape(pts.shape[0], -1)
            pts_canonical = self.deform_network(deform_code_embed, pts, alpha_ratio)
            ambient_coord = self.ambient_network(deform_code_embed, pts, alpha_ratio)
    ...
    def render(self, time_id, deform_code, appearance_code, rays_o, rays_d, near, far, perturb_overwrite=-1,
        ...
        if iter_step >= self.important_begin_iter and self.n_importance > 0:
            with torch.no_grad():
                ...
                pts = pts.reshape(-1, 3)
                # Deform
                deform_code_embed = deform_code(time_id.repeat(1,z_vals.shape[1]))
                ...
                for i in range(self.up_sample_steps):
                    new_z_vals = self.up_sample(rays_o,
                                                ...
                                                64 * 2**i)
                    z_vals, sdf = self.cat_z_vals(time_id,
                                                deform_code,
                                                  rays_o,
                                                  rays_d,
                                                  z_vals,
        ...
        # Render core
        ret_fine = self.render_core(time_id,
                                    deform_code,
                                    appearance_code,
                                    rays_o,
                                    rays_d,
