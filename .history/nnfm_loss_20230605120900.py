...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        # ouputs: pred      contents: origin      style: style_img
        for x in loss_names:
            assert x in ['nnfm_loss', 'content_loss', 'gram_loss']

        block_indexes = [[1, 3], [6, 8], [11, 13, 15], [18, 20, 22], [25, 27, 29],[15]]  # block_indexes is self-defined feature layer 
        ...
        for block in blocks:
            ...
            if "content_loss" in loss_names:
                ...
                loss_dict["content_loss"] += cos_loss(content_feats, x_feats)
