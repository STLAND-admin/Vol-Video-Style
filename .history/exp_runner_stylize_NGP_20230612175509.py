...
from models.fields import RenderingNetwork, SDFNetwork, SingleVarianceNetwork, DeformNetwork, NGPNetwork, AppearanceNetwork, TensorVMEncoding, TopoNetwork, DeformNetwork_MLP
...
class Runner:
    def __init__(self,dataset,  conf_path, mode='train', case='CASE_NAME', is_continue=False, is_paint = False, iswoab = False):
        ...
        self.TensorRF = TensorVMEncoding()
        # self.TensorRF = TensorRenderer(**self.conf['model.tensor_renderer']).to(self.device)
        ...
        if self.use_deform:
            ...
            elif self.mode == "style" or self.mode == "validate":
        ...
        if is_paint==False:
            ...
            params_to_train += [{'name':'sdf_network', 'params':self.sdf_network.parameters(), 'lr':self.learning_rate*0.5}]                   ## Named SDF but a MLP here
            # params_to_train += [{'name':'deform_network', 'params':self.deform_network.parameters(), 'lr':self.learning_rate}]
            # params_to_train += [{'name':'appearance_codes', 'params':self.appearance_codes.parameters(), 'lr':self.learning_rate}]
            # params_to_train += [{'name':'color_network', 'params':self.color_network.parameters(), 'lr':self.learning_rate}]
            params_to_train += [{'name':'ngp_color', 'params':self.ngp_color.parameters(), 'lr':self.learning_rate*10}]
            # params_to_train += [{'name':'TensorRF', 'params':self.TensorRF.parameters(), 'lr':self.learning_rate*10}]
        ...
        if is_continue:
            ...
            if is_paint == True and mode!='render':
                ...
                if self.mode == "style":
                    ckpts_path = ckpts_path.replace("_style_NGPStyle","_NeRF")
            # if mode == "validate":
        ...
        if style_ckpt is not None:
            latest_model_name = self.style_ckpt
    ...
    def load_checkpoint(self, checkpoint_name, ckpt_path):
        ...
        if self.use_deform:
            ...
            if self.ngp_color and 'ngp_color' in checkpoint.keys():
                self.ngp_color.load_state_dict(checkpoint['ngp_color'],strict=False)
    ...
    def style_train_all_image(self, resolution_level=-1):
        ...
        for image_idx in tqdm(range(self.dataset.n_images)):
            ...
            if iter < 10:
                ...
                print(f"content loss: {loss_dict['content_loss']} ", f"NNFM loss: { loss_dict['nnfm_loss']}",f"PSNR loss: { psnr}")
                # self.optimizer.zero_grad()
                loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--style_ckpt',type=str, default="")
