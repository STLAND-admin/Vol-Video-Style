...
class MLPRender_Fea(torch.nn.Module):
    def __init__(self,inChanel, viewpe=6, feape=0, featureC=128):
        super(MLPRender_Fea, self).__init__()

        self.in_mlpC = 2*feape*inChanel + 3 + inChanel # + 2*viewpe*3
        # self.viewpe = viewpe
        self.feape = feape
        layer0 = torch.nn.Linear(936, self.in_mlpC)
        layer1 = torch.nn.Linear(self.in_mlpC, featureC)
        layer2 = torch.nn.Linear(featureC, featureC)
        layer3 = torch.nn.Linear(featureC,3)

        self.mlp = torch.nn.Sequential(layer0,layer1, torch.nn.ReLU(inplace=True), layer2, torch.nn.ReLU(inplace=True), layer3)
...
class TensorVMEncoding(nn.Module):
    ...
    def __init__(
        ...
        super(TensorVMEncoding, self).__init__()
        ...
        self.color_net = MLPRender_Fea(inChanel=3, viewpe=6, feape=6, featureC=128)
    ...
    def forward(self, in_tensor) -> torch.Tensor:
        ...
        rgb = self.color_net(features)
        return rgb  # [..., 3 * Components]
...
class NGPNetwork(nn.Module):
    def __init__(self,
        ...
        self.encoder = tcnn.Encoding(
            ...
            encoding_config={
                ...
                "n_levels": 8,
                "n_features_per_level": 2, # ori=2
                "log2_hashmap_size": 19,
                "base_resolution": 16,
                "per_level_scale": per_level_scale,
            },
        )
        dims = [16] + [64 for _ in range(4)] + [3]



        # latent code
        self.color_net = MLP_v1(c_in = dims[0], c_out=3, c_hiddens=dims,zero_init=True)
    ...
    def forward(self, points):
        in_feature =self.encoder(points/np.pi)
        color = self.color_net(in_feature)
        return color
...
class NGPNetwork_encode(nn.Module):
    def __init__(self,
        ...
        self.net = tcnn.NetworkWithInputEncoding(n_input_dims=3, n_output_dims=3,encoding_config=self.encoder, network_config=self.color_net)


    def forward(self, points):
        points = self.net(points)
        return points
