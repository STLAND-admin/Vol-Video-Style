...
from models.fields import NGPNetwork_encode, RenderingNetwork, SDFNetwork, SingleVarianceNetwork, DeformNetwork, NGPNetwork, AppearanceNetwork, TopoNetwork, DeformNetwork_MLP
from models.renderer import NeuSRenderer, DeformNeRFRenderer, DeformNeRFNGPRenderer, DeformNeRFAppearanceRenderer
...
class Runner:
    def __init__(self,dataset,  conf_path, mode='train', case='CASE_NAME', is_continue=False, is_paint = False, iswoab = False):
        ...
        self.ngp_color = NGPNetwork_encode().to(self.device)
        ...
        if self.use_deform:
            if self.is_paint == True:
                ...
                self.renderer = DeformNeRFAppearanceRenderer(self.report_freq,
                                        self.deform_network,
                                        self.topo_network,
                                        self.sdf_network,
                                        self.deviation_network,
        ...
        if is_paint==False:
            ...
            params_to_train += [{'name':'ngp_color', 'params':self.ngp_color.parameters(), 'lr':self.learning_rate}]
        ...
        # os.makedirs(self.base_exp_dir, exist_ok=True)
        self.base_exp_dir = self.base_exp_dir + '_NGP'
        os.makedirs(self.base_exp_dir, exist_ok=True)
