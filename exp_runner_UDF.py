...
from models.fields import SingleBetaNetwork, RenderingNetwork, SDFNetwork, SingleVarianceNetwork, DeformNetwork, NGPNetwork, AppearanceNetwork, TopoNetwork, DeformNetwork_MLP
from models.renderer import NeuSRenderer
from models.renderer_udf import DeformUDFRenderer as DeformNeRFRenderer

from models.dataset_hyper import iPhoneDatasetFromAllFrames
from torch.utils.data import DataLoader
from models.UDF import UDFMLP
...
class Runner:
    def __init__(self,dataset,  conf_path, mode='train', case='CASE_NAME', is_continue=False, is_paint = False, iswoab = False):
        ...
        self.base_exp_dir = self.base_exp_dir + '_UDF'
        self.pre_image = 0
        ...
        self.sdf_network = UDFMLP(**self.conf['model.sdf_network']).to(self.device)

        self.deviation_network = SingleBetaNetwork(**self.conf['model.variance_network']).to(self.device)
        # Deform
        # if self.use_deform:
    ...
    def train(self):
        ...
        with tqdm(total=len(self.train_loader)) as t:
            for iter_i, data in enumerate(self.train_loader):
                ...
                # Loss
                eikonal_loss = gradient_o_error.mean()
